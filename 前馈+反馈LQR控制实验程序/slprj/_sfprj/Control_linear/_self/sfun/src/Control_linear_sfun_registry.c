...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_Control_linear_process_check_sum_call( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_Control_linear_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_Control_linear_process_testpoint_info_call( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_Control_linear_process_testpoint_info_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
unsigned int sf_process_autoinheritance_call( int nlhs, mxArray * plhs[], int
  nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_Control_linear_autoinheritance_info( int nlhs, mxArray *
  ...
  if (strcmp(machineName, "Control_linear") == 0) {
    const mxArray *newRhs[3] = { NULL, NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    newRhs[2] = prhs[3];
    return sf_Control_linear_autoinheritance_info(nlhs,plhs,3,newRhs);
  }
...
unsigned int sf_process_get_eml_resolved_functions_info_call( int nlhs, mxArray *
  plhs[], int nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_Control_linear_get_eml_resolved_functions_info( int
  ...
  if (strcmp(machineName, "Control_linear") == 0) {
    ...
    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_Control_linear_get_eml_resolved_functions_info(nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_Control_linear_method_dispatcher(SimStruct *S, unsigned
  ...
  method, void *data)
{
  if (!strcmp(machineName,"Control_linear")) {
    return(sf_Control_linear_method_dispatcher(simstructPtr,chartFileNumber,
...
extern void Control_linear_terminator(void);
...
extern void Control_linear_initializer(void);
extern void Control_linear_register_exported_symbols(SimStruct* S);
extern void Control_linear_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  Control_linear_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      Control_linear_debug_initialize();
    }

    Control_linear_initializer();
  }
